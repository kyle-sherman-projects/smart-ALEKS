# RCE Extraction + Spot-Check ( Hackweek — Phase 3 → 4)
...
_RCE_SpotCheck.html          # Reviewer QA gate: verify/sign-off + export verified JSON
...
2. **`_RCE_SpotCheck.html` (Phase 3 → 4 handoff).** Open in any browser. Loads
...
Then open `_RCE_SpotCheck.html` to see the reviewer gate (pre-loaded with the
ALEKS slice).
