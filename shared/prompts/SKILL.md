...
**Effect size type.** The platform uses two statistics, never Cohen's d, and
switches by study design — set `effect_size_type` accordingly:
- `pearson_r` for treatment-only correlative studies (ALEKS and most reports).
- `hedges_g` for experimental / quasi-experimental studies (cluster, quantile).
Infer from the analysis method if the label isn't explicit (correlative → r;
experimental/quasi-experimental → g), and apply the same type to subgroup rows
...
**Statistical significance.** Significance is judged from the confidence interval,
not a p-value (the platform doesn't report p-values). A finding is `significant`
when the report says so (headline text "statistically significant") **or** the CI
in the callout excludes 0 (e.g. `CI: (0.15, 0.34)` → significant;
`CI: (-0.07, 0.42)` → `not_significant`). The CI is a 95% interval by default —
record `confidence_level` (default `0.95`) and only change it if the report states
a different level. If neither text nor CI is available, use `not_reported`.
...
- `subgroup_category` — one of the six standardized categories: `school`,
  `grade_level`, `ell` (English as a Second Language), `gender`, `race_ethnicity`,
  `frl_status` (Free/Reduced Lunch). Anything else a district defines (e.g. SpEd
  status, prior-performance tiers, a program name) is `custom` — put the report's
  literal label in `subgroup_category_custom` (e.g. `"SpEd status"`,
  `"prior performance"`). Map the dashboard toggle names accordingly: "Ethnicity"
  → `race_ethnicity`, "SpEd Status" → `custom` + `"SpEd status"`, "Prior
  Performance" → `custom` + `"prior performance"`. Nothing here is required —
  reports vary; the only universally required item is a categorical usage metric.
- `subgroup_value` — the x-axis label (`"White"`, `"Female"`,
  `"Example School District Middle"`, `"No"`, `"Lowest 40% of Fall testing scores"`).
