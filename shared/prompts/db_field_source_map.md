...
Worked example: Example School District's i-Ready × Math Time on Task ×
SmarterBalanced had seven `active` rows (redacted per-grade ids), created 5–40 minutes
apart across about three hours on one day. That's someone iterating on the
setup, not a scheduled refresh. The last one in the sequence (`<redacted-id>`,
...
Worked example: The district's i-Ready trial (2025–26) has two outcome rows for
`"Math Time on Task"`:
