...
- **Source:** two `impact_analyses` rows for the district-wide diagnostic (iReady ELA and Math) plus seven rows each for SmarterBalanced Math and ELA (grades 3/4/5/6/7/8/10 — one row per grade, following the per-grade pattern documented in `shared/prompts/db_field_source_map.md` §§8–9). District and school names have been replaced with generic labels, consistent with the other briefs in this repository.
