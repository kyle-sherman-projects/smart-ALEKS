...
Status     : DRAFT v0.1 — for team review at  Hackweek Day 2
...
class EffectSizeType(str, Enum):
    """Effect-size statistic. The platform switches by study design (it does NOT
    use Cohen's d):
      - PEARSON_R : treatment-only correlative studies (e.g. our ALEKS reports).
      - HEDGES_G  : experimental / quasi-experimental studies (cluster, quantile).
    Significance is judged from the confidence interval, not a p-value."""
    PEARSON_R    = "pearson_r"
    HEDGES_G     = "hedges_g"
    NOT_REPORTED = "not_reported"
...
class SubgroupCategory(str, Enum):
    """Standardized subgroup categories plus CUSTOM. None are required on a report
    — districts may also define up to 5 custom (categorical) subgroups (e.g. SpEd
    status, prior-performance tiers, program names). Custom groups use CUSTOM with
    the district's label stored in SubgroupFinding.subgroup_category_custom."""
    SCHOOL         = "school"
    GRADE_LEVEL    = "grade_level"
    ELL            = "ell"             # English as a Second Language
    GENDER         = "gender"
    RACE_ETHNICITY = "race_ethnicity"
    FRL_STATUS     = "frl_status"      # Free or Reduced Lunch status
    CUSTOM         = "custom"          # district-defined; see subgroup_category_custom
...
class SubgroupFinding(BaseModel):
    ...
    subgroup_category: SubgroupCategory = Field(
        ...,
        description="Standardized subgroup category, or CUSTOM for district-defined groups."
    )
    subgroup_category_custom: Optional[str] = Field(
        None,
        description="District's label when subgroup_category is CUSTOM (e.g. 'SpEd status', 'prior performance'). None for standardized categories."
    )
    ...
    effect_size_type: Optional[EffectSizeType] = Field(
        None,
        description="[EXT] Effect-size statistic for this subgroup (pearson_r / hedges_g)."
    )
    statistical_significance: StatisticalSignificance = Field(
        StatisticalSignificance.NOT_REPORTED,
        description="[EXT] Statistical significance for this subgroup finding (95% CI excludes 0)."
    )
...
class RCEReport(BaseModel):
    ...
    effect_size_type: Optional[EffectSizeType] = Field(
        None,
        description="[EXT] Effect-size statistic: pearson_r (correlative) or hedges_g (experimental/quasi-experimental). Cohen's d is not used by the platform."
    )
    confidence_level: float = Field(
        0.95,
        description="[EXT] Confidence level of the CI used to judge significance. Platform default 0.95 (95%)."
    )
...
AMPLIFY_BOOST_EXAMPLE = RCEReport(
    ...
    subgroup_findings=[
        ...
        SubgroupFinding(
            effectiveness=EffectivenessDirection.POSITIVE,
            essa_tier=ESSATier.TIER_3,
            subgroup_category="race_ethnicity",
        ...
        SubgroupFinding(
            effectiveness=EffectivenessDirection.POSITIVE,
            essa_tier=ESSATier.TIER_3,
            subgroup_category="custom",
            subgroup_category_custom="SpEd status",
            subgroup_value="no",
            grade_levels=["3", "4", "5"],
            usage_metric="total_time",
            outcome_measure="Fastbridge aReading",
